...
│   ├── index.js              # 已有
│   ├── db/
│   │   ├── ident/            # 【去标识化数据】供报告/上云使用
│   │   │   ├── schema-ident.sql
│   │   │   └── charity-ident.db
│   │   ├── full/             # 【全量原始数据】严格保密，不上云
│   │   │   ├── schema-full.sql
│   │   │   └── charity-full.db
│   │   └── charity.db        # 主数据库（默认去标识化）
│   ├── routes/
│   │   ├── bookings.js       # 已有
│   │   ├── messages.js        # 已有
│   │   ├── resources.js      # 已有
...
1. **双库分离**（核心原则）
   - `db/ident/` 去标识化库 → 供报告生成、数据上云使用
   - `db/full/` 全量原始库 → 严格保密，仅本地存储，不上云
   - 两库定期同步（每周一次备份全量库）

2. **去标识化**：所有儿童信息严格匿名，姓名只存姓氏，电话只存尾号4位，省份代替详细地址

3. **隐私优先**：全量原始库严格保密，不上云，不给第三方

4. **提案导向**：每一个数据字段设计时都考虑"能否支撑政协/民盟提案"

5. **可量化**：所有描述尽可能数字化（XX%，XX次，XX人）

6. **报告按需导出**：不受月度/季度限制，每个报告含日期，可随时生成
